...
function ProductsContent() {
  ...
  const fetchProducts = useCallback(async () => {
    setLoading(true)
    try {
      // Fetch both products and official categories
      const [prodRes, catRes] = await Promise.all([
        fetch("/api/products", { cache: "no-store" } as RequestInit),
        fetch("/api/categories", { cache: "no-store" } as RequestInit)
      ])
      ...
      if (catJson.success) {
        const officialCats = catJson.data.map((c: any) => c.name)
        setCategories(["All", ...officialCats])
      } else if (prodJson.success) {
        // Fallback to deriving from products if categories API fails
        const uniqueCats = Array.from(new Set<string>(prodJson.data.map((p: Product) => p.mainCategory))) as string[]
